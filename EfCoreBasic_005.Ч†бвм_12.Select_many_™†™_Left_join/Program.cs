...
namespace EfCoreBasic_005.Часть_11.Select_many_как_Left_join
{
        ...
        public static void Чтение_данных()
        {
            ...
            var joinedCoursesQueryable =
                dbContext.Authors
                    .SelectMany(
                        ...
                        author => dbContext.Courses.Where(x => x.AuthorId == author.Id)
                            .DefaultIfEmpty(),
                        // результат объединения
                        (author, course) => new { Course = course, Author = author });
            ...
            foreach (var joinedCourse in joinedCourses)
            {
                Console.WriteLine(
                    ...
                    $"Имя курса: {joinedCourse.Course?.Name ?? "<null>"}. " +
                    $"Идентификатор курса: {joinedCourse.Course?.Id ?? -1}. " +
                    $"Количество уроков: {joinedCourse.Course?.LessonsQuantity ?? -1}.");
            }
